...
NailStream *stream = &tmpstream;dep_checksum=read_unsigned_bits(stream,16);
}n_tr_setpos(trace,trace_checksum);
n_tr_const(trace,str_current);
if(checksum_parse(tmp_arena,str_current,&dep_checksum)) {goto fail;}n_tr_stream(trace,str_current);if(parser_fail(stream_check(str_current,8))) {goto fail;}
...
if( read_unsigned_bits(str_current,8)!= 221){stream_backup(str_current,8);goto fail;}if(parser_fail(n_tr_const(trace,str_current))){goto fail;}
if(parser_fail(stream_check(str_current,8))) {goto fail;}
if( read_unsigned_bits(str_current,8)!= 165){stream_backup(str_current,8);goto fail;}if(parser_fail(n_tr_const(trace,str_current))){goto fail;}
if(parser_fail(stream_check(str_current,8))) {goto fail;}
stream_advance(str_current,8);
if(parser_fail(stream_check(str_current,8))) {goto fail;}
...
stream_advance(str_current,1);
if(parser_fail(stream_check(str_current,1))) {goto fail;}
if( read_unsigned_bits(str_current,1)!= 0){stream_backup(str_current,1);goto fail;}if(parser_fail(n_tr_const(trace,str_current))){goto fail;}
if(parser_fail(stream_check(str_current,1))) {goto fail;}
stream_advance(str_current,1);
if(parser_fail(stream_check(str_current,1))) {goto fail;}
stream_advance(str_current,1);
...
tr++;
*stream = *(NailStream *)tr;tr+= sizeof(NailStream) / sizeof(*tr);stream_reposition(stream,*tr);
tr++;
...
*stream = *(NailStream *)tr;tr+= sizeof(NailStream) / sizeof(*tr);stream_reposition(stream,*tr);
tr++;
*trace = tr;return 0;}cmd_req_t*parse_cmd_req_t(NailArena *arena, const uint8_t *data, size_t size){
NailStream stream = {.data = data, .pos= 0, .size = size, .bit_offset = 0};
NailArena tmp_arena;NailArena_init(&tmp_arena, 4096, arena->error_ret);n_trace trace;
pos *tr_ptr;
 pos pos;
cmd_req_t* retval;
n_trace_init(&trace,4096,4096);
if(parser_fail(peg_cmd_req_t(&tmp_arena,&trace,&stream))) goto fail;if(stream.pos != stream.size) goto fail; retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
stream.pos = 0;
tr_ptr = trace.trace;if(bind_cmd_req_t(arena,retval,&stream,&tr_ptr,trace.trace) < 0) goto fail;
out: n_trace_release(&trace);
NailArena_release(&tmp_arena);return retval;fail: retval = NULL; goto out;}
...
static int bind_motor_controller_cmd_req_t(NailArena *arena,motor_controller_cmd_req_t*out,NailStream *stream, pos **trace ,  pos * trace_begin){
 pos *tr = *trace;out->command=read_unsigned_bits(stream,8);
stream_reposition(stream,*tr);
tr++;
stream_reposition(stream,*tr);
tr++;
*stream = *(NailStream *)tr;tr+= sizeof(NailStream) / sizeof(*tr);*trace = tr;return 0;}motor_controller_cmd_req_t*parse_motor_controller_cmd_req_t(NailArena *arena, const uint8_t *data, size_t size){
NailStream stream = {.data = data, .pos= 0, .size = size, .bit_offset = 0};
NailArena tmp_arena;NailArena_init(&tmp_arena, 4096, arena->error_ret);n_trace trace;
pos *tr_ptr;
 pos pos;
motor_controller_cmd_req_t* retval;
n_trace_init(&trace,4096,4096);
if(parser_fail(peg_motor_controller_cmd_req_t(&tmp_arena,&trace,&stream))) goto fail;if(stream.pos != stream.size) goto fail; retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
stream.pos = 0;
tr_ptr = trace.trace;if(bind_motor_controller_cmd_req_t(arena,retval,&stream,&tr_ptr,trace.trace) < 0) goto fail;
out: n_trace_release(&trace);
NailArena_release(&tmp_arena);return retval;fail: retval = NULL; goto out;}
static int bind_motor_controller_error_status_t(NailArena *arena,motor_controller_error_status_t*out,NailStream *stream, pos **trace ,  pos * trace_begin){
 pos *tr = *trace;out->identity=read_unsigned_bits(stream,1);
out->battery_over_voltage=read_unsigned_bits(stream,1);
out->battery_under_voltage=read_unsigned_bits(stream,1);
stream_reposition(stream,*tr);
tr++;
out->locking=read_unsigned_bits(stream,1);
out->v_positive=read_unsigned_bits(stream,1);
out->controller_over_temp=read_unsigned_bits(stream,1);
stream_reposition(stream,*tr);
tr++;
out->reset=read_unsigned_bits(stream,1);
out->pedal=read_unsigned_bits(stream,1);
out->hall_sensor=read_unsigned_bits(stream,1);
out->emergency_rev=read_unsigned_bits(stream,1);
out->motor_over_temp=read_unsigned_bits(stream,1);
out->current_meter=read_unsigned_bits(stream,1);
*trace = tr;return 0;}motor_controller_error_status_t*parse_motor_controller_error_status_t(NailArena *arena, const uint8_t *data, size_t size){
NailStream stream = {.data = data, .pos= 0, .size = size, .bit_offset = 0};
NailArena tmp_arena;NailArena_init(&tmp_arena, 4096, arena->error_ret);n_trace trace;
pos *tr_ptr;
 pos pos;
motor_controller_error_status_t* retval;
n_trace_init(&trace,4096,4096);
if(parser_fail(peg_motor_controller_error_status_t(&tmp_arena,&trace,&stream))) goto fail;if(stream.pos != stream.size) goto fail; retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
stream.pos = 0;
tr_ptr = trace.trace;if(bind_motor_controller_error_status_t(arena,retval,&stream,&tr_ptr,trace.trace) < 0) goto fail;
out: n_trace_release(&trace);
NailArena_release(&tmp_arena);return retval;fail: retval = NULL; goto out;}
...
out->battery_voltage=read_unsigned_bits(stream,8);
out->motor_temperature=read_unsigned_bits(stream,8);
out->controller_temperature=read_unsigned_bits(stream,8);
out->setting_direction=read_unsigned_bits(stream,8);
...
pos *tr_ptr;
 pos pos;
motor_controller_monitor_1_resp_t* retval;
n_trace_init(&trace,4096,4096);
if(parser_fail(peg_motor_controller_monitor_1_resp_t(&tmp_arena,&trace,&stream))) goto fail;if(stream.pos != stream.size) goto fail; retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
stream.pos = 0;
...
NailOutStream_reposition(str_current, rewind_ntc_count);NailOutStream_write(str_current,dep_ntc_count,8);NailOutStream_reposition(str_current, end_of_struct);}return 0;}int gen_cell_voltage_resp_t(NailArena *tmp_arena,NailOutStream *str_current,cell_voltage_resp_t * val){uint8_t dep_cell_count;NailOutStreamPos rewind_cell_count=NailOutStream_getpos(str_current);NailOutStream_write(str_current,0,8);for(int i2=0;i2<val->cell_voltage.count;i2++){if(parser_fail(NailOutStream_write(str_current,val->cell_voltage.elem[i2],16))) return -1;}dep_cell_count=val->cell_voltage.count;{/*Context-rewind*/
 NailOutStreamPos  end_of_struct= NailOutStream_getpos(str_current);
NailOutStream_reposition(str_current, rewind_cell_count);NailOutStream_write(str_current,dep_cell_count,8);NailOutStream_reposition(str_current, end_of_struct);}return 0;}int gen_motor_controller_cmd_req_t(NailArena *tmp_arena,NailOutStream *str_current,motor_controller_cmd_req_t * val){if(parser_fail(NailOutStream_write(str_current,val->command,8))) return -1;if(parser_fail(NailOutStream_write(str_current,0,8))) return -1;uint8_t dep_checksum;NailOutStreamPos rewind_checksum=NailOutStream_getpos(str_current);NailOutStream_write(str_current,0,8);if(parser_fail(motor_controller_checksum_generate(tmp_arena,str_current,&dep_checksum))){return -1;}{ NailOutStreamPos t_rewind_eot =NailOutStream_getpos(str_current);
