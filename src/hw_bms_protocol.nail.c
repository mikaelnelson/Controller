...
static pos peg_cmd_resp_t( NailArena *tmp_arena, n_trace *trace, NailStream *str_current )
{
    ...
    if( read_unsigned_bits( str_current, 8 ) != 221 ) {
        stream_backup( str_current, 8 );
        goto fail;
    }
    if( parser_fail( n_tr_const( trace, str_current ))) { goto fail; }
    if( parser_fail( stream_check( str_current, 8 ))) { goto fail; }
    ...
    uint8_t dep_data_sz;
    {
        ...
        if( parser_fail( stream_check( str_current, 8 ))) { goto fail; }
        stream_advance( str_current, 8 );
        NailStream *stream = &tmpstream;
        dep_data_sz = read_unsigned_bits( stream, 8 );
    }
    ...
    uint16_t dep_checksum;
    {
        NailStream tmpstream = *str_current;
        if( parser_fail( stream_check( str_current, 16 ))) { goto fail; }
        stream_advance( str_current, 16 );
        NailStream *stream = &tmpstream;
        dep_checksum = read_unsigned_bits( stream, 16 );
    }
    n_tr_setpos( trace, trace_checksum );
    n_tr_const( trace, str_current );
    if( checksum_parse( tmp_arena, str_current, &dep_checksum )) { goto fail; }
    n_tr_stream( trace, str_current );
    if( parser_fail( stream_check( str_current, 8 ))) { goto fail; }
    if( read_unsigned_bits( str_current, 8 ) != 119 ) {
        stream_backup( str_current, 8 );
        goto fail;
    }
    if( parser_fail( n_tr_const( trace, str_current ))) { goto fail; }
...
static pos peg_cmd_req_t( NailArena *tmp_arena, n_trace *trace, NailStream *str_current )
{
    ...
    if( parser_fail( n_tr_const( trace, str_current ))) { goto fail; }
    if( parser_fail( stream_check( str_current, 8 ))) { goto fail; }
    if( read_unsigned_bits( str_current, 8 ) != 165 ) {
        stream_backup( str_current, 8 );
        goto fail;
    }
    if( parser_fail( n_tr_const( trace, str_current ))) { goto fail; }
...
static pos peg_fet_control_status_t( NailArena *tmp_arena, n_trace *trace, NailStream *str_current )
{
    ...
    if( read_unsigned_bits( str_current, 6 ) != 0 ) {
        stream_backup( str_current, 6 );
        goto fail;
    }
    if( parser_fail( n_tr_const( trace, str_current ))) { goto fail; }
    if( parser_fail( stream_check( str_current, 1 ))) { goto fail; }
...
static pos peg_basic_status_resp_t( NailArena *tmp_arena, n_trace *trace, NailStream *str_current )
{
    ...
    uint8_t dep_ntc_count;
    {
        ...
        stream_advance( str_current, 8 );
        NailStream *stream = &tmpstream;
        dep_ntc_count = read_unsigned_bits( stream, 8 );
    }
...
static pos peg_cell_voltage_resp_t( NailArena *tmp_arena, n_trace *trace, NailStream *str_current )
{
    ...
    uint8_t dep_cell_count;
    {
        ...
        stream_advance( str_current, 8 );
        NailStream *stream = &tmpstream;
        dep_cell_count = read_unsigned_bits( stream, 8 );
    }
...
static pos bind_cmd_resp_t( NailArena *arena, cmd_resp_t *out, NailStream *stream, pos **trace, pos *trace_begin );

static pos bind_cmd_req_t( NailArena *arena, cmd_req_t *out, NailStream *stream, pos **trace, pos *trace_begin );

static pos bind_protection_status_t( NailArena *arena, protection_status_t *out, NailStream *stream, pos **trace, pos *trace_begin );

static pos bind_fet_control_status_t( NailArena *arena, fet_control_status_t *out, NailStream *stream, pos **trace, pos *trace_begin );

static pos bind_basic_status_resp_t( NailArena *arena, basic_status_resp_t *out, NailStream *stream, pos **trace, pos *trace_begin );

static pos bind_cell_voltage_resp_t( NailArena *arena, cell_voltage_resp_t *out, NailStream *stream, pos **trace, pos *trace_begin );
...
static int bind_cmd_resp_t( NailArena *arena, cmd_resp_t *out, NailStream *stream, pos **trace, pos *trace_begin )
{
    ...
    { /*ARRAY*/
        ...
        for( pos i1 = 0; i1 < out->data.count; i1++ ) {
            out->data.elem[i1] = read_unsigned_bits( stream, 8 );
        }
        tr = trace_begin + save;
    }
    stream_reposition( stream, *tr );
    tr++;
    *stream = *(NailStream *) tr;
    tr += sizeof( NailStream ) / sizeof( *tr );
    stream_reposition( stream, *tr );
    tr++;
    *trace = tr;
    return 0;
...
cmd_resp_t *parse_cmd_resp_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    if( parser_fail( peg_cmd_resp_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_cmd_resp_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
...
static int bind_cmd_req_t( NailArena *arena, cmd_req_t *out, NailStream *stream, pos **trace, pos *trace_begin )
{
    pos *tr = *trace;
    stream_reposition( stream, *tr );
    tr++;
...
cmd_req_t *parse_cmd_req_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    cmd_req_t *retval;
    n_trace_init( &trace, 4096, 4096 );
    if( parser_fail( peg_cmd_req_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_cmd_req_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
    n_trace_release( &trace );
    NailArena_release( &tmp_arena );
    return retval;
...
protection_status_t *parse_protection_status_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    n_trace_init( &trace, 4096, 4096 );
    if( parser_fail( peg_protection_status_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_protection_status_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
    n_trace_release( &trace );
    NailArena_release( &tmp_arena );
    return retval;
...
fet_control_status_t *parse_fet_control_status_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    n_trace_init( &trace, 4096, 4096 );
    if( parser_fail( peg_fet_control_status_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_fet_control_status_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
    n_trace_release( &trace );
    NailArena_release( &tmp_arena );
    return retval;
...
{
    pos *tr = *trace;
    out->total_voltage = read_unsigned_bits( stream, 16 );
    ...
    if( parser_fail( bind_protection_status_t( arena, &out->protection_status, stream, &tr, trace_begin ))) { return -1; }
    out->version = read_unsigned_bits( stream, 8 );
    out->rsoc = read_unsigned_bits( stream, 8 );
    if( parser_fail( bind_fet_control_status_t( arena, &out->fet_control_status, stream, &tr, trace_begin ))) { return -1; }
    out->cell_series_count = read_unsigned_bits( stream, 8 );
    stream_reposition( stream, *tr );
    tr++;
    { /*ARRAY*/
        pos save = 0;
        out->ntc_temperature.count = *(tr++);
        save = *(tr++);
        out->ntc_temperature.elem = (typeof( out->ntc_temperature.elem )) n_malloc( arena, out->ntc_temperature.count *
                                                                                           sizeof( *out->ntc_temperature.elem ));
        for( pos i2 = 0; i2 < out->ntc_temperature.count; i2++ ) {
            out->ntc_temperature.elem[i2] = read_unsigned_bits( stream, 16 );
        }
        tr = trace_begin + save;
    }
    *trace = tr;
...
basic_status_resp_t *parse_basic_status_resp_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    n_trace_init( &trace, 4096, 4096 );
    if( parser_fail( peg_basic_status_resp_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_basic_status_resp_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
    n_trace_release( &trace );
    NailArena_release( &tmp_arena );
    return retval;
...
static int bind_cell_voltage_resp_t( NailArena *arena, cell_voltage_resp_t *out, NailStream *stream, pos **trace, pos *trace_begin )
{
    ...
    { /*ARRAY*/
        pos save = 0;
        out->cell_voltage.count = *(tr++);
        save = *(tr++);
        out->cell_voltage.elem = (typeof( out->cell_voltage.elem )) n_malloc( arena, out->cell_voltage.count *
                                                                                     sizeof( *out->cell_voltage.elem ));
        for( pos i3 = 0; i3 < out->cell_voltage.count; i3++ ) {
            out->cell_voltage.elem[i3] = read_unsigned_bits( stream, 16 );
        }
        tr = trace_begin + save;
    }
    *trace = tr;
...
cell_voltage_resp_t *parse_cell_voltage_resp_t( NailArena *arena, const uint8_t *data, size_t size )
{
    ...
    n_trace_init( &trace, 4096, 4096 );
    if( parser_fail( peg_cell_voltage_resp_t( &tmp_arena, &trace, &stream ))) { goto fail; }
    if( stream.pos != stream.size ) { goto fail; }
    retval = (typeof( retval )) n_malloc( arena, sizeof( *retval ));
    stream.pos = 0;
    tr_ptr = trace.trace;
    if( bind_cell_voltage_resp_t( arena, retval, &stream, &tr_ptr, trace.trace ) < 0 ) { goto fail; }
    out:
    n_trace_release( &trace );
    NailArena_release( &tmp_arena );
    return retval;
...
extern int checksum_generate( NailArena *tmp_arena, NailOutStream *str_current, uint16_t *checksum );

int gen_cmd_req_t( NailArena *tmp_arena, NailOutStream *out, cmd_req_t *val );
...
int gen_cmd_resp_t( NailArena *tmp_arena, NailOutStream *str_current, cmd_resp_t *val )
{
    if( parser_fail( NailOutStream_write( str_current, 221, 8 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->command, 8 ))) { return -1; }
    if( val->state != 0 && val->state != 128 ) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->state, 8 ))) { return -1; }
    uint8_t dep_data_sz;
    NailOutStreamPos rewind_data_sz = NailOutStream_getpos( str_current );
    NailOutStream_write( str_current, 0, 8 );
    for( int i0 = 0; i0 <
                     val->data.count; i0++ ) { if( parser_fail( NailOutStream_write( str_current, val->data.elem[i0], 8 ))) { return -1; }}
    dep_data_sz = val->data.count;
    uint16_t dep_checksum;
    NailOutStreamPos rewind_checksum = NailOutStream_getpos( str_current );
    NailOutStream_write( str_current, 0, 16 );
    if( parser_fail( NailOutStream_write( str_current, 119, 8 ))) { return -1; }
    if( parser_fail( checksum_generate( tmp_arena, str_current, &dep_checksum ))) { return -1; }
    {
        NailOutStreamPos t_rewind_eot = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, rewind_checksum );
        NailOutStream_write( str_current, dep_checksum, 16 );
        NailOutStream_reposition( str_current, t_rewind_eot );
    }
    {/*Context-rewind*/
        NailOutStreamPos end_of_struct = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, rewind_data_sz );
        NailOutStream_write( str_current, dep_data_sz, 8 );
        NailOutStream_reposition( str_current, rewind_checksum );
        NailOutStream_write( str_current, dep_checksum, 16 );
...
int gen_cmd_req_t( NailArena *tmp_arena, NailOutStream *str_current, cmd_req_t *val )
{
    if( parser_fail( NailOutStream_write( str_current, 221, 8 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, 165, 8 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->command, 8 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, 0, 8 ))) { return -1; }
    uint16_t dep_checksum;
    NailOutStreamPos rewind_checksum = NailOutStream_getpos( str_current );
    NailOutStream_write( str_current, 0, 16 );
    if( parser_fail( NailOutStream_write( str_current, 119, 8 ))) { return -1; }
...
int gen_protection_status_t( NailArena *tmp_arena, NailOutStream *str_current, protection_status_t *val )
{
    if( parser_fail( NailOutStream_write( str_current, 0, 3 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->mosfet_software_lock, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->ic_frontend_error, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->short_circuit, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->discharge_overcurrent, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->charge_overcurrent, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->discharge_low_temp, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->discharge_high_temp, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->charge_low_temp, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->charge_high_temp, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->battery_under_voltage, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->battery_over_voltage, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->cell_under_voltage, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->cell_over_voltage, 1 ))) { return -1; }
    {/*Context-rewind*/
        NailOutStreamPos end_of_struct = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, end_of_struct );
    }
...
int gen_fet_control_status_t( NailArena *tmp_arena, NailOutStream *str_current, fet_control_status_t *val )
{
    if( parser_fail( NailOutStream_write( str_current, 0, 6 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->is_charging, 1 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->is_discharging, 1 ))) { return -1; }
    {/*Context-rewind*/
        NailOutStreamPos end_of_struct = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, end_of_struct );
    }
...
int gen_basic_status_resp_t( NailArena *tmp_arena, NailOutStream *str_current, basic_status_resp_t *val )
{
    if( parser_fail( NailOutStream_write( str_current, val->total_voltage, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->current, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->residual_capacity, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->nominal_capacity, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->cycle_life, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->product_date, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->balance_status, 16 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->balance_status_high, 16 ))) { return -1; }
    if( parser_fail( gen_protection_status_t( tmp_arena, str_current, &val->protection_status ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->version, 8 ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->rsoc, 8 ))) { return -1; }
    if( parser_fail( gen_fet_control_status_t( tmp_arena, str_current, &val->fet_control_status ))) { return -1; }
    if( parser_fail( NailOutStream_write( str_current, val->cell_series_count, 8 ))) { return -1; }
    uint8_t dep_ntc_count;
    NailOutStreamPos rewind_ntc_count = NailOutStream_getpos( str_current );
    NailOutStream_write( str_current, 0, 8 );
    for( int i1 = 0; i1 <
                     val->ntc_temperature.count; i1++ ) { if( parser_fail( NailOutStream_write( str_current, val->ntc_temperature.elem[i1], 16 ))) { return -1; }}
    dep_ntc_count = val->ntc_temperature.count;
    {/*Context-rewind*/
        NailOutStreamPos end_of_struct = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, rewind_ntc_count );
        NailOutStream_write( str_current, dep_ntc_count, 8 );
        NailOutStream_reposition( str_current, end_of_struct );
    }
    return 0;
}

int gen_cell_voltage_resp_t( NailArena *tmp_arena, NailOutStream *str_current, cell_voltage_resp_t *val )
{
    uint8_t dep_cell_count;
    NailOutStreamPos rewind_cell_count = NailOutStream_getpos( str_current );
    NailOutStream_write( str_current, 0, 8 );
    for( int i2 = 0; i2 <
                     val->cell_voltage.count; i2++ ) { if( parser_fail( NailOutStream_write( str_current, val->cell_voltage.elem[i2], 16 ))) { return -1; }}
    dep_cell_count = val->cell_voltage.count;
    {/*Context-rewind*/
        NailOutStreamPos end_of_struct = NailOutStream_getpos( str_current );
        NailOutStream_reposition( str_current, rewind_cell_count );
        NailOutStream_write( str_current, dep_cell_count, 8 );
        NailOutStream_reposition( str_current, end_of_struct );
    }
    return 0;
}
